...
def main():
    ...
    selected_app_url = st.sidebar.selectbox("Select App", list(apps.keys()))

    if st.sidebar.button("Go to App"):
        selected_app_url = apps[selected_app_url]
        st.markdown(f"[Go to App]({selected_app_url})", unsafe_allow_html=True)
